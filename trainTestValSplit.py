...
def getCSVs(_dataset_path: str) -> set[str, pd.DataFrame]:
    paths_CSV: set[str] = set([join(_dataset_path, f) for f in listdir(_dataset_path) if isfile(join(_dataset_path, f))])
    return {path_CSV: pd.read_csv(path_CSV) for path_CSV in paths_CSV}


def getTrainTestValSplit(_dataset_path: str, _train_frac: float = 0.7, _test_frac: float = 0.2, _val_frac: float = 0.1) -> list[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    CSVs: set[str, pd.DataFrame] = getCSVs(_dataset_path=_dataset_path)
    assert _train_frac + _test_frac + _val_frac <= 1.0

    train_filenames, test_filenames = train_test_split(list(CSVs.keys()), test_size=_test_frac, train_size=_train_frac, random_state=7342)
